...
import { episodes, getEpisode } from '@/lib/data'
...
export default function EpisodePage({ params }: Props) {
  ...
  const related = episodes.filter(e =>
    e.slug !== ep.slug &&
    e.topics.some(t => ep.topics.includes(t))
  ).slice(0, 2)
    ...
    <>
      ...
      <div className="pt-24 bg-espresso">
        <div className="max-w-7xl mx-auto px-6 lg:px-10 py-12 lg:py-20">
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            {/* Meta */}
            <div>
              <Link href="/episodes" className="text-xs text-copper/70 uppercase tracking-widest hover:text-copper transition-colors mb-6 inline-flex items-center gap-2">
                ← All Episodes
              </Link>
              <div className="flex flex-wrap gap-2 mb-5">
                {ep.topics.map(t => (
                  <span key={t} className="inline-block text-xs uppercase tracking-widest px-3 py-1 rounded-full bg-white/10 text-copper/80 border border-copper/20">
                    {t.replace(/-/g, ' ')}
                  </span>
                ))}
              </div>
              <h1 className="font-serif text-3xl lg:text-5xl font-bold text-cream leading-tight mb-4">
                {ep.title}
              </h1>
      ...
      <div className="bg-espresso border-t border-white/10">
        <div className="max-w-7xl mx-auto px-6 lg:px-10 py-4 flex flex-wrap gap-3">
          {ep.spotifyUrl && (
            <a href={ep.spotifyUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-copper border border-copper/20 px-4 py-2 rounded-full hover:bg-copper/10 transition-colors">
              Listen on Spotify
            </a>
          )}
          <a href="https://podcasts.apple.com" target="_blank" rel="noopener noreferrer" className="text-xs text-copper border border-copper/20 px-4 py-2 rounded-full hover:bg-copper/10 transition-colors">
            Apple Podcasts
          </a>
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-12">
                ...
                <div className="space-y-3">
                  {ep.takeaways.map((t, i) => (
                    <div key={i} className="flex gap-4 bg-beige-light rounded-xl p-4">
                      <span className="font-serif text-2xl font-bold text-copper/40 leading-none mt-0.5">
            ...
            {ep.resources.length > 0 && (
              <div>
                <h2 className="font-serif text-2xl font-bold text-espresso mb-5">Resources</h2>
                <div className="space-y-3">
                  {ep.resources.map((r, i) => (
                    <div key={i} className="border border-beige/60 rounded-xl p-4 bg-white/60">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <span className="topic-pill text-[10px] mb-2">{r.type}</span>
                          <h3 className="font-serif font-semibold text-espresso text-sm">{r.title}</h3>
                          {r.author && <p className="text-xs text-warm-gray">{r.author}</p>}
                          {r.description && <p className="text-xs text-warm-gray mt-1">{r.description}</p>}
                        </div>
                        {r.url && (
                          <a href={r.url} target="_blank" rel="noopener noreferrer" className="shrink-0 text-xs text-terracotta hover:underline">
                            View →
                          </a>
                        )}
                      </div>
            ...
            <div>
              ...
              <div className="bg-beige-light rounded-xl p-6 text-sm text-warm-gray leading-relaxed">
                {ep.transcript || 'Full transcript coming soon.'}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white/60 border border-beige/60 rounded-2xl p-6">
              ...
              <Link href={`/guests`} className="text-xs text-terracotta hover:underline">
                View full profile →
              </Link>
            </div>
            ...
            <div className="bg-white/60 border border-beige/60 rounded-2xl p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {ep.topics.map(t => (
                  <Link key={t} href={`/topics/${t}`} className="topic-pill hover:bg-terracotta hover:text-cream transition-colors">
                    {t.replace(/-/g, ' ')}
                  </Link>
                ))}
              </div>
