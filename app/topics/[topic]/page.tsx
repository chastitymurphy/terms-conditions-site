...
export default async function TopicPage({ params }: { params: { topic: string } }) {
  ...
  const [allEpisodes, allReflections] = await Promise.all([getEpisodes(), getReflections()])
  const topicEpisodes    = allEpisodes.filter(ep => ep.topics.includes(params.topic))
  const topicReflections = allReflections.filter(r => r.topic === params.topic)

  return (
    <>
      <div className="pt-32 pb-16 bg-espresso">
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          <div className="flex items-center gap-3 mb-4"><div className="h-px w-8 bg-copper/60" /><span className="text-xs font-sans uppercase tracking-[0.2em] text-copper/80">Topic</span></div>
